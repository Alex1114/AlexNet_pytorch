...
class ImageCreator():
    ...
    def __init__(self):
        ...
        f1 = open('./train.txt', 'w') # change "ele" to the folder name you created/
        f2 = open('./test.txt', 'w') 
        ...
        with rosbag.Bag('1.bag', 'r') as bag: #open first .bag
            ...
            for topic,msg,t in bag.read_messages():
                #print topic
                if topic == "/super_pi06/joy_mapper_node/car_cmd": #change ros_master name from aiplus1 to your duckiebot's name
                    #print topic, msg.header.stamp
                    self.omega = msg.omega
                    self.v = msg.v
                    self.t = 1
                elif topic == "/super_pi06/camera_node/image/compressed":
                    if self.t == 1:
                        try:
                            #print topic, msg.header.stamp
                        ...
                        if self.i == 9:
                            f2.write("./dataset/joystick/"+image_name+" "+str(self.omega)+"\n")
                            self.i = 0
                        else:                             
                            f1.write("./dataset/joystick/"+image_name+" "+str(self.omega)+"\n")
                            self.i += 1
